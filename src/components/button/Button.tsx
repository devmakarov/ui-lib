...
const Button = forwardRef<HTMLButtonElement, UIButton>((props, ref) => {
    const { type, size, disabled = false, onClick, children, style: customStyles, className = '' } = props
    ...
    return (
        <button className={classNames} style={customStyles} disabled={disabled} ref={ref} onClick={handleClick}>
            {children}
        </button>
    )
})
